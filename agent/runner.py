...
INTENT_SCHEMA = {
    "type": "object",
    "properties": {
        "tool": {"type": "string", "enum": ["create_transaction", "create_transfer", "get_month_summary", "get_balance", "get_category_summary", "get_transactions"]},
...
def dispatch(intent: dict, context: AgentContext) -> str:
    ...
    if tool == "create_transaction":
        return client.call("POST", "transactions", intent, context=context)
    if tool == "create_transfer":
        return client.call("POST", "transfers", intent, context=context)
    ...
    if tool == "get_category_summary":
        ...
        return client.call("GET", f"category-summary?{urlencode({'category_name': intent.get('category_name', '')})}", context=context)
...
def main() -> int:
    ...
    result = dispatch(intent, AgentContext(provider="whatsapp", sender_id=args.sender, message_id=args.message_id))
    print(json.dumps({"intent": intent, "backend": json.loads(result)}, ensure_ascii=False, indent=2))
    return 0
