...
import { money, monthLabel, normalizeMoneyInput, numberValue, percent, todayForTimezone } from "@/lib/utils";
...
export default function BudgetsPage() {
  ...
  const create = async () => { setSaving(true); setFormError(null); try { const normalizedLimit = normalizeMoneyInput(limit); if (!categoryId || !normalizedLimit || Number(normalizedLimit) <= 0) throw new Error("Escolha uma categoria e informe um limite monetário válido."); await api.createBudget({ category_id: categoryId, month, limit_amount: normalizedLimit }); setFormOpen(false); setLimit(""); await load(); } catch (reason) { setFormError(reason instanceof Error ? reason.message : "Não foi possível criar o orçamento."); } finally { setSaving(false); } };
  return <>
