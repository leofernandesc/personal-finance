...
function FlowChart({ data }: { data: DashboardData["flow"] }) {
  ...
  return <ResponsiveContainer width="100%" height="100%"><AreaChart data={chartData} margin={{ top: 10, right: 8, left: -22, bottom: 0 }}><defs><linearGradient id="incomeFill" x1="0" y1="0" x2="0" y2="1"><stop offset="5%" stopColor="#2f6b58" stopOpacity={0.2} /><stop offset="95%" stopColor="#2f6b58" stopOpacity={0} /></linearGradient><linearGradient id="expenseFill" x1="0" y1="0" x2="0" y2="1"><stop offset="5%" stopColor="#b95d45" stopOpacity={0.14} /><stop offset="95%" stopColor="#b95d45" stopOpacity={0} /></linearGradient></defs><CartesianGrid vertical={false} stroke="#e4e5e1" strokeDasharray="3 3" /><XAxis dataKey="label" tickLine={false} axisLine={false} tick={{ fill: "#6d7782", fontSize: 10 }} minTickGap={24} /><YAxis tickLine={false} axisLine={false} tick={{ fill: "#6d7782", fontSize: 10 }} tickFormatter={(value) => `R$${value}`} /><Tooltip contentStyle={{ borderRadius: 12, border: "1px solid #e4e5e1", boxShadow: "0 10px 30px rgba(24,58,77,.08)", fontSize: 12 }} formatter={(value) => money(Number(value))} /><Area type="monotone" dataKey="income" name="Entradas" stroke="#2f6b58" strokeWidth={2.2} fill="url(#incomeFill)" /><Area type="monotone" dataKey="expense" name="Saídas" stroke="#b95d45" strokeWidth={2.2} fill="url(#expenseFill)" /></AreaChart></ResponsiveContainer>;
}
