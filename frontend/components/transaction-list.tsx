...
export function TransactionList({ transactions, compact = false }: { transactions: Transaction[]; compact?: boolean }) {
  ...
  return <div className="divide-y divide-line">{transactions.map((transaction) => { const accountLabel = transaction.type === "transfer" ? `${transaction.transfer_source_account_name || transaction.account_name || "Origem"} → ${transaction.transfer_destination_account_name || "Destino"}` : transaction.account_name || transaction.account?.name || "Conta"; return <div key={transaction.id} className="flex items-center gap-3 px-5 py-3.5 md:px-6"><TransactionMark type={transaction.type} /><div className="min-w-0 flex-1"><p className="truncate text-sm font-semibold text-ink">{transaction.description || (transaction.type === "income" ? "Receita" : transaction.type === "transfer" ? "Transferência" : "Despesa")}</p><p className="mt-0.5 truncate text-xs text-muted">{transaction.category_name || transaction.category?.name || (transaction.type === "transfer" ? "Transferência" : "Sem categoria")} <span className="px-1 text-line">·</span> {accountLabel}</p></div><div className="hidden text-right sm:block"><p className={`text-sm font-semibold ${transaction.type === "income" ? "text-moss" : transaction.type === "expense" ? "text-ink" : "text-muted"}`}>{transaction.type === "income" ? "+" : transaction.type === "expense" ? "−" : ""}{money(transaction.amount)}</p><p className="mt-0.5 text-[0.68rem] text-muted">{formatDate(transaction.transaction_date)} {compact ? "" : `· ${sourceLabels[transaction.source] || transaction.source}`}</p></div><div className="text-right sm:hidden"><p className={`text-sm font-semibold ${transaction.type === "income" ? "text-moss" : transaction.type === "transfer" ? "text-muted" : "text-ink"}`}>{transaction.type === "income" ? "+" : transaction.type === "expense" ? "−" : ""}{money(transaction.amount)}</p><p className="mt-0.5 text-[0.68rem] text-muted">{formatDate(transaction.transaction_date)}</p></div></div>; })}</div>;
}
